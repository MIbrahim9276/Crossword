...
class CrosswordCreator():
    ...
    def ac3(self, arcs=None):
        ...
        if arcs is None:
            for x_var in self.domains:
                for y_var in self.domains:
                    if x_var != y_var and self.crossword.overlaps[x_var, y_var]:
    ...
    def order_domain_values(self, var, assignment):
        ...
        for value in self.domains[var]:
            ...
            for neighbor in self.crossword.neighbors(var):
                (i, j) = self.crossword.overlaps[var, neighbor]
                for z_value in self.domains[neighbor]:
                    if value[i] != z_value[j]:
                        count += 1
            counts[value] = count

        return list({k: v for k, v in sorted(counts.items(), key=lambda item: item[1])}.keys())
